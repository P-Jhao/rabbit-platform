...
// 获取分类列表数据
const categoryListRef = ref<CategoryTopItem[]>([])
...
//提取当前二级分类数据
const subCategoryListRef = computed(() => {
  return categoryListRef.value[activeIndexRef.value]?.children || []
})
</script>
  ...
  <view class="viewport" v-else>
    ...
    <view class="categories">
      ...
      <scroll-view class="primary" scroll-y>
        <view
          v-for="(item, index) in categoryListRef"
          :key="item.id"
          class="item"
          :class="{ active: activeIndexRef === index }"
          @tap="() => (activeIndexRef = index)"
        >
      ...
      <scroll-view class="secondary" scroll-y>
        <!-- 焦点图 -->
        <XtxSwiper class="banner" :list="bannerListRef" />
        <!-- 内容区域 -->
        <view class="panel" v-for="item in subCategoryListRef" :key="item.id">
          <view class="title">
            <text class="name">{{ item.name }}</text>
