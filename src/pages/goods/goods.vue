...
//获取商品详情数据
const goodsResultRef = ref<GoodsResult>()
...
const handleSwiperChange: UniHelper.SwiperOnChange = (e) => {
  currentIndexRef.value = e.detail.current
}
// 处理大图预览
const handleImgTap = (url: string) => {
  uni.previewImage({
    current: url,
...
// 处理弹出层
const popupNameRef = ref<'service' | 'address'>()
const popup = ref<{
  open: (type?: UniHelper.UniPopupType) => void
  close: () => void
}>()
const handleOpenTap = (name: typeof popupNameRef.value) => {
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    <view class="goods">
      ...
      <view class="preview">
        <swiper circular @change="handleSwiperChange">
          <swiper-item v-for="(item, index) in goodsResultRef?.mainPictures" :key="index">
            <image mode="aspectFill" :src="item" @tap="handleImgTap(item)" />
          </swiper-item>
        </swiper>
        <view class="indicator">
          <text class="current">{{ currentIndexRef + 1 }}</text>
          <text class="split">/</text>
          <text class="total">{{ goodsResultRef?.mainPictures.length }}</text>
        </view>
      </view>
      ...
      <view class="meta">
        <view class="price">
          ...
          <text class="number">{{ goodsResultRef?.price }}</text>
        </view>
    ...
    <view class="detail panel">
      ...
      <view class="content">
        <view class="properties">
          ...
          <view
            class="item"
            v-for="(item, index) in goodsResultRef?.details.properties"
            :key="index"
          >
        ...
        <image
          mode="widthFix"
          :src="item"
          v-for="(item, index) in goodsResultRef?.details.pictures"
          :key="index"
        >
        </image>
      </view>
    ...
    <view class="similar panel">
      ...
      <view class="content">
        <navigator
          v-for="item in goodsResultRef?.similarProducts"
          :key="item.id"
          class="goods"
          hover-class="none"
  ...
  <uni-popup ref="popup" type="bottom" background-color="white">
    <AddressPanel v-show="popupNameRef === 'address'" @close="popup?.close" />
    <ServicePanel v-show="popupNameRef === 'service'" @close="popup?.close" />
  </uni-popup>
</template>
